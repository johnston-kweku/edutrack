const roles = document.querySelectorAll('#roles span')
const roleInput = document.getElementById('id_role')


roles.forEach(role => {
    role.addEventListener('click', () => {
        roles.forEach(role => role.classList.remove('bg-blue-200'))
        role.classList.add('bg-blue-200')
        if(role.id === 'admin') {
            roleInput.value = 'ADMIN'
        }else if(role.id === 'teachingStaff') {
            roleInput.value = 'TEACHING_STAFF'
